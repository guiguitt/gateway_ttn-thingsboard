...
def PostThingsboard( JsDATA, DeviceId):
    ...
    ResponseAcessToken = requests.post(dPrm['THINGS']['url']+'/api/auth/login/public', headers=headers_AcessT, data='{"publicId":"'+ dPrm['THINGS']['publicid']+'"}')
    if 'status' in ResponseAcessToken.text: 
        printlog(ResponseAcessToken.text)
    else:
        printlog("2/9 - Request response ok")
    ...
    if 'status' in ResponseDeviceToken.text:
        ...
        printlog("5/9 - Request response ok")
